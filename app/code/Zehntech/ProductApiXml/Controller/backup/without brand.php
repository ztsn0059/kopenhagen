      ...
      if($fileData){
         ...
         $head = array_flip($head);
          ...
          foreach ($fileData as $value => $row) 
          {
            ...
            $popular = $row[$head['Popular']] ? $row[$head['Popular']]:0;
            $new = $row[$head['New']] ? $row[$head['New']]:0;
            $offer = $row[$head['Offer']] ? $row[$head['Offer']]:0;
                ...
                if($supplier=="Remmer"){ 
                  ...
                  try{
                    ...
                    $product->save();
                      ...
                      $arr_product[] = $product->getSku();
                      $this->_logger->info("product of sku - " . $product->getSku() . " is not save 1st");
                  }catch(\Exception $e){
                    $arr_product[] = $product->getSku();
                    $this->_logger->info("Product - " . $product->getSku() . " is not save 2nd");
                  }
