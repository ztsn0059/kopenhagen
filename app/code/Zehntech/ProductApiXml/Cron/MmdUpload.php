<?php
  ...
  use Zehntech\ProductApiXml\Helper\MmdParser;
  ...
  class MmdUpload
  {
    ...
    public function execute()
    {
      ...
      $itemData = $fileData['ProduktInfoClass'];
        if($fileData){
      // print_r($itemData);
      // die("die");
      $count = 0;
      ...
      {
        $sku = $singleData['MANSKU'];
        $name = utf8_decode(strip_tags($singleData['PROD']));
        $cost = $singleData['PRICE'];
        // $popular = ($singleData['Popular']=='Yes') ? 1:0;
        // $new = ($singleData['New']=='Yes') ? 1:0;
         $clientId = array_key_exists('ClientId', $singleData) ? $singleData['ClientId'] : "";
        $barcode = array_key_exists('EAN', $singleData) ? $singleData['EAN'] : ""; 
        ...
        $oem = array_key_exists('OEMNR', $singleData) ? $singleData['OEMNR'] : 0;
        ...
        $minSales = array_key_exists('MOQ', $singleData) ? $singleData['MOQ'] : 0;
        ...
        $headCategoryText =  array_key_exists('GROUP', $singleData) ? utf8_decode(strip_tags($singleData['GROUP'])) : 0;
        ...
        $categoryText = array_key_exists('SUBGROUP', $singleData) ? utf8_decode(strip_tags($singleData['SUBGROUP'])) : 0;
        ...
        $quantity = array_key_exists('QTY', $singleData) ? $singleData['QTY'] : 0;
